...
def test_inference_log_format_helpers():
    ...
    end_line = build_end_log(success=True, score=0.45, steps=3, rewards=[0.1, 0.2, 0.45])
    ...
    assert end_line == "[END] success=true score=0.45 steps=3 rewards=0.11,0.20,0.45"
...
def test_end_log_is_single_line_and_uses_two_decimal_rewards():
    end_line = build_end_log(success=False, score=0.89, steps=2, rewards=[0.11, 0.89])

    assert "\n" not in end_line
    assert end_line == "[END] success=false score=0.89 steps=2 rewards=0.11,0.89"


def test_end_log_defaults_to_safe_reward_when_rewards_empty():
    end_line = build_end_log(success=False, score=0.0, steps=0, rewards=[])

    assert "\n" not in end_line
    assert end_line == "[END] success=false score=0.11 steps=0 rewards=0.11"
